...
<head>
    ...
    <title>SHOESTEP - Tambah Produk</title>
    @fonts
...
<body class="bg-[#F7F6F3] text-[#1D1B18] antialiased">
    ...
    <main class="mx-auto max-w-4xl px-6 py-10">
        <div class="mb-6">
            ...
            <h1 class="mt-2 text-3xl font-black text-slate-950">Tambah Produk Baru</h1>
        </div>
                ...
                <div class="grid gap-0 divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
                    ...
                    <div class="space-y-5 p-6">
                        <div>
                            ...
                            <input id="name" name="name" value="{{ old('name') }}" required maxlength="150"
                        ...
                        <div>
                            ...
                            <input id="slug" name="slug" value="{{ old('slug') }}" required maxlength="180"
                                class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-slate-400 focus:bg-white focus:outline-none">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input id="stock" name="stock" type="number" min="0" step="1" value="{{ old('stock', 0) }}" required
                                    class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-slate-400 focus:bg-white focus:outline-none">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input id="weight" name="weight" type="number" min="0" step="0.01" value="{{ old('weight') }}"
                                    class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-slate-400 focus:bg-white focus:outline-none">
                            </div>
                            <div>
                    ...
                    <div class="space-y-5 p-6">
                        <div>
                            <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Upload Thumbnail</label>
                            <div id="drop-zone"
                                 onclick="document.getElementById('thumbnail_file').click()"
                                 class="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-sm text-slate-500 transition hover:border-slate-400 hover:bg-white">
                                <img id="thumb-preview" src="" alt="" class="hidden h-24 w-24 rounded-xl object-cover mb-1">
                                <svg id="thumb-icon" xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                                <span id="thumb-label">Klik untuk pilih gambar</span>
                            ...
                            <input id="thumbnail" name="thumbnail" value="{{ old('thumbnail', 'products/') }}" maxlength="255"
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" required rows="7"
                                class="w-full resize-y rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-slate-400 focus:bg-white focus:outline-none">{{ old('description') }}</textarea>
                        </div>
                        ...
                        <div class="flex gap-6">
                            ...
                            <label class="inline-flex cursor-pointer items-center gap-2 text-sm font-semibold text-slate-700">
                                <input type="checkbox" name="popular" value="1" @checked(old('popular'))
                                    class="h-4 w-4 rounded border-slate-300 accent-slate-900">
                                Popular
                            </label>
                        </div>
