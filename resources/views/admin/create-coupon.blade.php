...
<head>
    ...
    <title>SHOESTEP - Tambah Promo</title>
    @fonts
...
<body class="bg-[#F7F6F3] text-[#1D1B18] antialiased">
    ...
    <main class="mx-auto max-w-3xl px-6 py-10">
        <div class="mb-6">
            ...
            <h1 class="mt-2 text-3xl font-black text-slate-950">Tambah Kode Promo / Diskon</h1>
        </div>
            ...
            <form method="POST" action="{{ route('admin.coupons.store') }}">
                ...
                <div class="space-y-5 p-6">
                    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                        <div>
                            <label for="code" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Kode Kupon * (otomatis kapital)</label>
                            <input id="code" name="code" value="{{ old('code') }}" required maxlength="50"
                        ...
                        <div>
                            ...
                            <select id="discount_type" name="discount_type" required onchange="updateDiscountLabel()"
                                class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-slate-400 focus:bg-white focus:outline-none">
                                <option value="percentage" @selected(old('discount_type', 'percentage') === 'percentage')>Persentase (%)</option>
                        ...
                        <div>
                            ...
                            <input id="discount_value" name="discount_value" type="number" min="0" step="0.01" value="{{ old('discount_value', 0) }}" required
                                class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-slate-400 focus:bg-white focus:outline-none">
                        </div>
                        ...
                        <div>
                            ...
                            <input id="minimum_order" name="minimum_order" type="number" min="0" step="0.01" value="{{ old('minimum_order', 0) }}" required
                                class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-slate-400 focus:bg-white focus:outline-none">
                        </div>

                        <div>
                            <label for="usage_limit" class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Batas Penggunaan</label>
                            <input id="usage_limit" name="usage_limit" type="number" min="1" step="1" value="{{ old('usage_limit') }}"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input id="expired_at" name="expired_at" type="datetime-local" value="{{ old('expired_at') }}"
                                class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-slate-400 focus:bg-white focus:outline-none">
                            <p class="mt-1 text-[11px] text-slate-400">Kosongkan jika tidak ada batas waktu.</p>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="description" name="description" rows="3"
                                placeholder="Contoh: Diskon 50% untuk pembelian pertama"
